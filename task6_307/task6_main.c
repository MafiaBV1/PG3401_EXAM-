/*
 * Task6: EWP-over-TCP network client
 */
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
static void sendAck(int iSockFd, unsigned int uiSeqNum, int iNack) {
	...
	stAck.usChecksum       = calcChecksum((unsigned char *)&stAck,
	                             (int)(sizeof(stAck) - sizeof(stAck.Data)));
	send(iSockFd, &stAck, iAckSize, 0);
	printf("Sent ACK: ack=%u flags=0x%02x bytes=%d\n",
	       uiSeqNum, stAck.ucFlags, iAckSize);
}
...
int main(int argc, char *argv[])
{
	...
	while (!iDone) {
		...
		/* Swap sequence number to host byte order */
		...
		sendAck(iSockFd, stHead.uiSequenceNumber + (unsigned int)iDataSize, 0);
